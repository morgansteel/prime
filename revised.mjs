...
function askLucas() {
    ...
    getLucas.question("Enter an exponent to test", (input) => {
...
function askSPRP() {
    ...
    const getSPRP = createInterface({
        input: process.stdin,
        output: process.stdout
    });

    getSPRP.question("Enter an odd number to test", (input) => {
        let choice = parseInt(input);
        STRONG_PROBABLE_PRIME(choice);
        getSPRP.close();
        UI_BUILDER();
...
function UI_BUILDER() {
    ...
    console.log("[5] Run the Miller test, a speedup of the SPRP test");
...
function SIEVE(choice) {
    ...
    for (let i = 2; i < Math.ceil(Math.sqrt(choice)); i++) {
        // The sieve only needs to be done up to the square root of the limit n
        if (TEST_NUMS[i] == true) {
            // Every time a prime number is encountered, mark all multiples of it as false
    ...
    for (let j = 0; j <= choice; j++) {
        // Iterate through all items in the Boolean-sorted array
        if (TEST_NUMS[j] == true) {
            // Pick only those confirmed to be prime
            primes.push(j); // Add it to the list
        }
    }
    console.log("Sieved primes" + primes);
    UI_BUILDER();
    return primes; // Keep list for use in TF
}

function LUCAS_LEHMER(choice) {
...
function STRONG_PROBABLE_PRIME(choice) {
    ...
    for (let a = 2; a < choice; a++) {
        // Definition of the SPRP test

        if (a ** d % choice !== 1) {
            // If at any point the residue does not equal 1, the SPRP test guarantees it's composite
    ...
    if (isComposite) {
        ...
        console.log(
            "Prime! Checking against list of known pseudoprimes to increase confidence...",
        );

        for (let i = 0; i < PSEUDO_PRIMES.length; i++) {
            // Linear sort algorithm to find a match in the list
            ...
            if (PSEUDO_PRIMES[i] == choice) {
                ...
                // It's guaranteed if N < 20,000 after checking for known pseudoprimes that N is prime
...
function WRITE_N_AS_PRODUCT(choice) {
    ...
    while (d % 2 === 0) {
        // If d is odd, then N is even, and thus not prime. No further testing would be required.
...
function MILLER(choice) {
    ...
    for (
        let a = 2;
        a < Math.ceil(2 * Math.pow(Math.log(parseInt(choice)), 2));
        a++
    ) {
        // Per the E-RH, the bound is reduced to 2(log n)^2
    ...
    if (isComposite) {
        ...
        console.log(
            "Suspected Prime! Checking against list of known pseudoprimes to increase confidence...",
        );
